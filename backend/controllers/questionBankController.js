...
const mapAiQuestion = (q, defaultTopic, defaultKelas, penalaranLogis) => {
    // 1. Extract Question Text (check standard English/Indonesian keys)
    let questionText = q.question || q.soal || q.pertanyaan || q.text || q.questionText || q.question_text || q.prompt;
    if (!questionText) {
        // Fallback: look for any string field with significant length
        const stringFields = Object.values(q).filter(val => typeof val === 'string');
        questionText = stringFields.find(str => str.length > 20) || '';
    }
    if (!questionText || questionText.trim() === '') {
        questionText = "Soal Asesmen HOTS NARA-AI";
    }

    // 2. Extract Options
    let opts = q.options || q.pilihan || q.opsi || q.choices || q.answers || q.opt;
    if (!Array.isArray(opts) || opts.length !== 4) {
        // Fallback: look for any array of 4 items
        const arrays = Object.values(q).filter(val => Array.isArray(val));
        const fourItemArray = arrays.find(arr => arr.length === 4);
        if (fourItemArray) {
            opts = fourItemArray;
        } else {
            opts = ['A', 'B', 'C', 'D'];
    ...
    let correctVal = q.correct !== undefined ? q.correct : (q.kunci !== undefined ? q.kunci : (q.jawaban !== undefined ? q.jawaban : (q.correctAnswer !== undefined ? q.correctAnswer : (q.correct_index !== undefined ? q.correct_index : 0))));
    let correctIndex = 0;
    ...
    // Handle 1-based index from AI if correctIndex is 1 to 4 and q.options doesn't contain this number
    if (correctIndex >= 1 && correctIndex <= 4 && (!q.options || !q.options.includes(correctIndex))) {
    ...
    const explanationText = q.explanation || q.pembahasan || q.penjelasan || q.reason || q.discussion || 'Jawaban benar adalah opsi ' + (['A', 'B', 'C', 'D'][correctIndex]);

    // 5. Extract Type
    const typeVal = q.type || q.tipe || '';
    const finalType = String(typeVal).toLowerCase().includes('num') ? 'numerasi' : 'literasi';
    ...
    return {
        ...
        type: finalType,
        topic: q.topic || defaultTopic || 'Generated by AI',
        grade: '7 SMP',
        curriculum: 'Fase D',
        difficulty: 'HOTS',
        penalaranLogis: penalaranLogis || 3,
        kelas: defaultKelas || 'Semua Kelas'
    };
};
...
exports.generateFromAI = async (req, res) => {
    try {
        ...
        const questionsToInsert = generatedQuestions.map(q => mapAiQuestion(q, 'Generated by AI', kelas, levelPenalaran));
...
exports.generateFromMaterial = async (req, res) => {
    try {
        ...
        const questionsToInsert = generatedQuestions.map(q => mapAiQuestion(q, material.name, kelas, levelPenalaran));
